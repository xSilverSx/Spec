...
1. В любое удобное место скачать файлы *(находятся в папке "Спецификация")*, по ссылке "releases" версия может быть не актуальна: 
...
3. Далее работа ведется с файлом **"ШаблонSpec.xlsx"**, при открытии документа Excel есть  кнопка "Создать спецификацию" на вкладке "Спецификация".
