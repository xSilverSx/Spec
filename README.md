...
1. В любое удобное место скачать файлы: 
...
3. Далее работа ведется с файлом ***"Шаблон для спецификации.xlsx"*** .
4. Файл ***Open list.txt*** после этого можно удалить, если его оставить паралельно с открытием файлов эксель будет открываться пустой лист. Но встречается проблема когда при работе надстройки не видны таблицы Экселя, открытие этого пустого листа позволяет избежать данную проблему, в этом случае оставляем лежать этот файл вместе с ***надстройкой***.
5. Для обновления просто заменяем файл надстройки новым.
## Описание программы
...
### СО и ВР
Эти листы подготавливают для печати то, что заполнили на листе спецификация. Они разлинованы под ГОСТ, за исключением таблиц слева, они привязаны к ячейкам таблицы Excel, к сожалению, не нашел простого способа сделать эти таблицы по ГОСТу.
...
5.	В случае если столбцы "Завод изготовитель", "Примечание" длиннее (по строчкам) то необходимо заполнить пустые клетки в колонках "код оборудования", "Завод изготовитель" нулями "0" (на листах для печати автоматически стираются) 
...
2.	После добавление элементов повторно нажимаем кнопку «Включить», чтобы скрыть лист базы данных
3.	Кнопка «Сортировать базу»
4.	Кнопка «Сохранить книгу» чтобы пересохранить надстройку (иначе новые элементы не сохранятся).
